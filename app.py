...
# ── KML Parsing ───────────────────────────────────────────────────────────────
FILE_ID = "1s6RmISyV7d59H3FE7y_vPl-NZlRnI4xR"
...
def download_gdrive(file_id):
    ...
    # Newer Google Drive uses a form with a confirm param in the HTML
    if confirm_token is None and b"confirm=" in response.content:
...
@st.cache_data(ttl=3600)  # re-fetches every 1 hour
def load_kml(file_id):
    content = download_gdrive(file_id)
    # Safety check — make sure we got XML, not an HTML error page
    if not content.strip().startswith(b"<?xml") and not content.strip().startswith(b"<kml"):
        raise ValueError("Google Drive did not return a valid KML file. Check sharing permissions (must be 'Anyone with the link').")
...
try:
    ...
    st.error(f"⚠️ Could not load KML from Google Drive: {e}")
    st.stop()
